...
function ExperienceCard({ item }) {
  return (
    <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-6 lg:p-8 transition-all duration-300 hover:border-lime-400/20">
      <span className="inline-block px-4 py-1.5 text-xs text-lime-400 rounded-full mb-4" style={{ backgroundColor: "rgba(163,230,53,0.1)" }}>{item.duration}</span>
      ...
      <div className="flex flex-wrap gap-2">
        {item.technologies.map((tech) => (
          <span key={tech} className="px-3 py-1.5 text-xs text-gray-400 rounded-lg" style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.05)" }}>{tech}</span>
        ))}
      </div>
    </div>
  );
              ...
              return (
                <div key={item.id} data-exp-card className="relative">
                  ...
                  <div className="exp-row-desktop">
                    {isEven ? (
                      <><div className="text-right pr-10"><ExperienceCard item={item} /></div><div /></>
                    ) : (
                      <><div /><div className="pl-10"><ExperienceCard item={item} /></div></>
                    )}
                  </div>
                </div>
              );
